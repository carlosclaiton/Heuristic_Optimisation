...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [BestxVar,BestCost,Best]=BS_Qlearning(NumAgents,NUM_ITERATIONS,CostFunction,noV)
...
for j=1:NUM_ITERATIONS
    ...
    for i=1:NumAgents
        ...
        if i == indexBest
            rewardVal = 1;
            Q(i) = Q(i) + alpha*(rewardVal+gamma*max(Q) - Q(i));

        else
            rewardVal = 0;
            Q(i) = Q(i) + alpha*(rewardVal+gamma*max(Q) - Q(i));
            %The action will be select a random position to flipp
            indFlip = randi([1,noV]);
